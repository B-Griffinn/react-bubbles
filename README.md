...
Tokens are used to validate a users credentials upon entering any given page that requires a token.
Tokens are specicific to each user. The server holds these tokens within the users account so that only the user assigned the specific token can login with those credentials. 

- [X] What steps can you take in your web apps to keep your data secure?
There are two layers of protection added to our Application:
1) Protected/Private Routes
2) Authentication headers objects --> found in our axiosWithAuth file.

- [X] Describe how web servers work?
A web server is a computer that stores code/data for a specified website OR it is a simple program running on a computer, such as a local server like the one in todays project. 

- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers?
Create (axios.create) --> axiosWithAuth is an example
Read AKA GET (axios.get)
Update AKA PUT (axios.put)
Delete/Destroy 
